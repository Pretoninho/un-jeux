...
> - **Discipline à tenir** : ne **jamais mélanger logique et affichage** ; c'est ce qui garde toutes les portes ouvertes.
>
> **✅ TRANCHÉ (2026-06-14) — le moteur de rendu reste SVG.** Le choix n'est plus différé. Raison : le jeu rend
> ~13-16 hexes *statiques* + jauges + barres de signaux + data-viz + texte — un profil **tour par tour, UI-lourd**,
> sans action temps réel ni milliers d'entités. Dans ce profil, un moteur « jeu » (Pixi/Phaser) est un **contre-emploi** :
> il impose une boucle de rendu impérative qui rompt la réactivité déclarative de Svelte, et complique texte/data-viz/
> accessibilité, pour un seul gain réel — les particules. Donc :
> - **Défaut = SVG** (`App.svelte`, déjà en place). Le « juice » (remplissage de jauge animé, pulsation rouge quand `F`
>   monte, glow d'hexe, fondus de révélation) passe par **CSS + transitions/motion Svelte** : suffisant, gratuit en archi.
> - **Escape hatch ciblé** : si le moment de **krach** mérite un jour des particules GPU, ajouter **un overlay PixiJS sur
>   ce seul effet** — une couche, pas une migration ; jamais un framework qui avale l'UI.
> - **Unity** = réservé à un éventuel virage **build natif** (desktop/mobile/console), via portage du moteur en C# cadré
>   par les 60+ tests. Hors sujet tant qu'on est web.
> - **Direction artistique** (palette / iconographie / mood / textures) est **orthogonale** au moteur de rendu : elle
>   nourrit n'importe lequel et constitue un **chantier à part**, à mener indépendamment.
...
| 2026-06-13 | **J7 CALIBRAGE LIVRÉ (code, §28)** : tempo réglé par les paramètres générateurs uniquement, jamais le timing. **Cause racine** : `accValuation × stretch × 100` dominait l'accumulation (~0.10-0.19/tour) → F pilotée par les IA, **levier noyé**, crise quasi certaine dès le tour 4. **Rééquilibrage** (tout en plages, `params.ts`) : valorisation ramenée au niveau levier/crowding (`accValuation` 0.0010-0.0020), **levier relevé en moteur** (`accLeverage` 0.08-0.16), **purge élargie** (`purgeMeanReversion` 0.020-0.058 = variance de pente → la branche sans-crise existe), `crisisK` adouci (0.7-1.1), `f0` élargi (0.08-0.36), drifts bull/tension abaissés, cascades raccourcies (3-6 t), horizon 13-16. **Cibles §28.2 atteintes** (800 parties/profil, multi-profils §28.8) : sans-crise **24 %**, crise<t5 **<1 %**, F(t6)~0.50, drawdown **23-58 %**, **signaux>horloge §28.7**, doubles pyromanes **~11 %** (lev4). Le **comportement façonne la distribution** (hoarder 38 % calme/1 % double · pyromane 6 %/11 %). Instrument `scripts/calibrate.ts` (`npx vite-node`), garde anti-régression `src/engine/calibration.test.ts` (6 assertions). **68 tests, typecheck + build OK** |
| 2026-06-13 | **§23.5 ASSOUPLI (décision design, J7)** : le reset post-crise n'est plus « quasi-total ». `resetFactor` relevé à **0.32-0.48** (F revient vers ~0.25-0.38, juste sous la zone morte) pour que les **tables pyromanes rallument une 2ᵉ crise** (cible 2 crises 10-15 % §28.2, atteinte ~11 % sur lev4). Arbitrage explicitement choisi : on sacrifie un peu du principe « la crise purge le système » au profit du double-krach. Levier quasi-orthogonal au taux de parties calmes (n'affecte que les parties déjà en crise) → sans-crise resté à 24 % |
| 2026-06-13 | **J7 CLOS — neutralité §28.8 + α + viabilité levier (code)** : 2ᵉ cause racine trouvée — **`fluxImpact` ~100× trop fort** (flux en termes absolus O(100) avec START_CAPITAL=100, paramètre réglé pour O(1)) → l'achat normal des IA saturait le rendement au plafond +50 %/tour, **marché auto-pompé**, le fonds leveragé chevauchait sa propre bulle (+2930 % de score, **99 % de victoires** → défaut #4 inversé : levier god-tier). **Corrections** : `fluxImpact` → 0.0002-0.0006 (un ordre normal bouge ~1-2 %, une fire-sale ~10 % — l'impact-prix §25.4 garde des dents pour la contagion sans pomper) ; levier **carry-neutre** (`leverageBorrowRate` 0.03-0.05 ≈ carry, sinon le carry encaissé sur le notionnel leveragé > coût d'emprunt = revenu sans risque qui compose) ; **drifts quasi nuls** (`driftBull` 0-0.010, `driftTension` 0.003-0.015 → marché ≈ martingale + krachs, le levier amplifie un pari à skew négatif, pas un gain garanti, §29.3) ; **α (`drawdownPenalty`) = 0.35 FIXE** (score transparent §27.3) centre le duel. **Résultats** (800 parties/profil) : agrégat tempo **28/59/13** · **duel levier>value 46 %** (neutralité §28.8 ✓, chaque profil top1 ~44 %, hoarder gagne 11 % via les krachs) · drawdowns mordent (lev4 94 %) · signaux>horloge §28.7 ✓. **Effet de structure** : sans le pump de valorisation, F est piloté par le COMPORTEMENT (levier+crowding) → un monde sans levier ne devient pas fragile (bimodalité assumée, conforme §23.2). **8 assertions** `calibration.test.ts`, **70 tests**, typecheck+build OK. **Toutes les vérifs numériques J7 (§28.2/§28.7/§28.8, α, levier) sont closes** |
| 2026-06-13 | **Crédit TRAVERSABLE + nœud Banque centrale UTILE (UI/exploration)** : deux manques couplés relevés au test. (1) « Se déplacer (sans investir) » manquait sur le crédit — `deplacer` était verrouillé par `canOpen`→`isInvestable` (marché ET non-crédit), donc le token ne pouvait PAS poser le pied sur un hexe crédit : murs infranchissables, et la **FED** (adjacente à IG_US crédit + LIQ) quasi inatteignable par le chemin naturel. → garde `canMoveTo` (traversable = marché V OU crédit, **frontière crédit verrouillée HY_US comprise** — on traverse mais l'**ouverture** reste bloquée par la frontière). (2) `hasNode('reglementaire')` n'était jamais lu alors que sa description promet « présence = lecture anticipée des taux ». → **info-edge (phase 2a, spec §4c)** : sous présence FED, l'UI révèle `bc.target` (« Cible BC ✨ ») un tour à l'avance (forward guidance). UI : halo « openable » sur le crédit, bouton dédié, ligne de présence enrichie |
| 2026-06-13 | **BC par RÉUNIONS PLANIFIÉES (idée créateur, ACTIVÉE)** : la BC ne statue sur `r_BC` qu'aux **réunions** (param `bcMeetingEvery` tiré **4-5**), **DÉCISIVE** (saut à la cible, `smoothing=1` en mode réunions ; `bcReact` prend un θ optionnel), taux **FIGÉ entre deux**. `credit.bcMeets(turn, every)`, gate dans `turn.ts`. Mesuré (`scripts/bc-cadence.ts`, cadences 1/4/5/6 + variante décisive) : **(A)** distribution de crises / neutralité **identiques au bit près** → la BC **lit F, ne la pilote pas** (couche richesse) ; **(B)** dégrade le « 4ᵉ signal » de façon contrôlée (tracking r_BC~F 0.51→0.36, pred crise≤2t 0.38→0.20 graduel, **récupéré à ~0.30 en décisif**) — la staleness inter-réunions est l'intérêt, et le **forward guidance (présence FED)** devient un vrai edge. Conséquence : test d'intégration « la BC resserre en surchauffe » épinglé en mode continu (avec réunions la surchauffe peut tomber entre deux réunions) + nouveau test « figé entre réunions, bouge aux réunions ». UI : « 🏛️ Réunions tous les Nt · prochaine dans Kt », « Décision réunion ✨ » sous présence. **Infra** : `ConfigPartie.paramsOverride` (surcharge post-tirage, n'altère pas le flux du monde) → calibrage/expérience sans toucher au seed |
| 2026-06-13 | **PATH B — CAMPAGNE multi-cycles (décision créateur)** : la partie n'est plus UN cycle mais une SUITE de booms-busts. `horizonTurns` **r(13,16) → r(28,40)**. Mesuré (`scripts/horizon.ts`, balayage 15→40t) : étendre déplace mécaniquement la distribution (la **neutralité §28.8 tient à tout horizon** ~50 % duel ; la **branche sans-crise s'effondre** 25 %→6 % — pilier 1-cycle qui meurt au-delà de ~24t). Nouveaux repères agrégés (28-40t) : **sans ~8 % · 1 crise ~46 % · 2+ ~45 % · ~1.4 crise/partie**, date très étalée (σ ~8t → §28.7 OK). **Bandes §28.2/§28.8 re-ciblées** dans `calibration.test.ts` : quota calme 0.06-0.40 (au lieu de 0.30-0.70) ; « pyromane brûle 2× » → vérifie l'ORDRE (≫ prudent) + seuil multi-cycles ; **plancher top1 réserve 0.05→0.02** (la réserve pure rate trop de compounding → devient une **tactique défensive**, pas un win-con co-égal). Bénéfice : le sous-jeu crédit/duration/BC respire (~7-9 réunions/campagne). PATH A (horizon + accumulation ralentie pour garder le 1-cycle) mesuré et faisable, écarté |
| 2026-06-13 | **CARRY DU CASH EN RÉSERVE (réhabilite le hoarder en campagne)** : sur 28-40t, la réserve pure ratait tout le compounding (hoarder top1 3.5 %, score −92 %) → « le hoarder peut perdre » basculait en « perd presque toujours ». Idée créateur explorée et **mesurée large** (`scripts/cash-carry.ts`, panel taux × franchise × plafond) **avant tout code**. Constats : la **cannibalisation des hexes carry est un SEUIL DE TAUX** — invisible tant que le cash effectif < bande carry (LC ~1.5-2 %) ; `r_BC` moyen ~1.4 % la respecte, ×1.5 la viole (value top1 47 %→18 %). La **franchise** (R_min) est un levier orthogonal : protège le carry-holder + ne paie qu'un hoarding ASSUMÉ. Recette retenue (**conservatrice, choix créateur**) : `cashCarryFloor` r(40,60) entier (~0.5× capital), cash gagne **plein r_BC** au-dessus → `turn.ts` (4ter) `accrueCashCarry` après la réaction BC, pour TOUS les acteurs (neutre), **sans toucher F**. Effet mesuré (600 parties) : hoarder top1 **3.5 %→9.2 %** (re-win-con) mais **score moyen −62 %** (garde-fou intact : gagne les krachs, coule au calme) · value 47 %→**38 %** (cannibalisation bornée) · duel **55 %** (§28.8 ✓) · crises **inchangées** (couche richesse). Synergie : sous présence FED on anticipe le `r_BC` qui paie le cash, et la BC **resserre en surchauffe** → la patience mieux payée pile avant le krach. UI : « 💰 Poudre sèche +x/t » / « 💤 sous la franchise ». **99 tests verts, svelte-check 0, build OK** |
| 2026-06-13 | **ARCHÉTYPES — prêts à concevoir + règle des pouvoirs (réflexion)** : on est prêt (physique neutre calibrée + archi N-archétypes data-driven + un pouvoir prouvé de bout en bout `ignoreLockup`). Matière première abondante : chaque mécanique récente est une **surface de lentille** (carry cash→Vautour, BC→Sismographe macro, coupons→distressed, lockup→spécialiste immo, plafonds levier→fonds). **Gaps avant un roster complet** : victoires différenciées (AUM/domination/science) non implémentées → commencer par des archétypes à victoire « Score » ; ressources (`RESERVER` no-op, « réserve sèche » = label) à câbler ; badges (§7) = système à part. Méthode **un à la fois** (§30). **RÈGLE DES POUVOIRS (clé)** : un pouvoir peut/doit être EXAGÉRÉ en magnitude mais NEUTRE EN ESPÉRANCE — asymétrie (fort dans une dimension) ≠ dominance (gagne partout). 3 formes sûres : force CONDITIONNELLE (énorme en niche, négative ailleurs), contrainte appariée (§6), coût d'opportunité. Seul anti-pattern : **buff plat inconditionnel sur une grandeur qui COMPOSE = free lunch** impossible à équilibrer (rabote = tue le fantasme). On mesure le top1 et on renforce le CONTRE-POIDS, pas le rabotage |
| 2026-06-13 | **MÉTHODE « mesurer avant de coder » — validée sur un pouvoir game-breaking** : 1ère idée de pouvoir testée (compétence 3 PA → ×boost du carry pendant x tours) mesurée AVANT code (`scripts/archetype-carry-skill.ts`) → **catastrophique** : ×2 → top1 82 %, ×5/×8 → 100 %, excédent jusqu'à +900 000 %. Cause = multiplicateur sur un carry qui se RÉINVESTIT (compounding) = explosion exponentielle ; coût PA négligeable. **Cooldown long** (idée créateur) testé : bride bien (×2 : 82 %→51 % à cd16, ~2 usages) mais **insuffisant seul** — révèle que le **baseline « coupon long » gagne déjà 41-48 % SANS pouvoir**. La mesure-avant-code a épargné un pouvoir cassé ET exhumé un déséquilibre de fond |
| 2026-06-13 | **DÉSÉQUILIBRE CRÉDIT — faits puis Fix A (méthode : fix 1 à 1 jusqu'à neutralité)** : le coupon-long pur gagne ~48 % (vs marché +78 % médian ; actions −17 %, réserve −61 %). **4 causes** établies par mesure (`scripts/credit-baseline.ts`) : (1) compounding quasi sans risque au calme (0 crise→ ×8, mais 2+ crises→ ×0.3, blow-up 26 % → conditionnel mais branche calme démesurée) ; (2) les coupons ESQUIVENT le frein F (pas de levier → ne touchent que le petit terme crowding, pas le terme levier dominant) ; (3) **cause inattendue** : le harvester ÉTOUFFE les crises — `levier agrégé = emprunté / richesse TOTALE`, sa richesse-coupon qui compose gonfle le dénominateur → ratio dilué → F monte moins → sans-crise 12 %→31 %, boucle auto-renforçante ; (4) coupons non disputés par les IA (`couponAppetite` 0.2/0). **Fix A (adopté, suffisant seul)** : exclure la richesse-coupon du dénominateur du levier (`portfolio.aggregateLeverageRatio`, flag `fixLeverageDenom` défaut 1). Mesuré (`scripts/credit-fixes.ts`) : baseline coupon **48 %→31 %** (bande neutre), suppression de crises tuée (13/52/34 quand il joue), **calibration intouchée** (jeu normal sans coupons → dénominateur ~inchangé : PATH B 8/46/46, duel 55 %, 99 tests). Fixes B (reach-for-yield→F) et C (compression de rendement) prototypés en flags mais **NON nécessaires** (laissés off). Leçon : la cause racine la plus profonde (artefact de normalisation) était la plus simple à corriger |
| 2026-06-14 | **VAUTOUR — 1ᵉʳ vrai pouvoir d'archétype « Récolte » (data-driven, mesuré-avant-codé)** : compétence ACTIVE — coût 3 PA → multiplie le carry encaissé (positions V + coupons) par `factor` pendant `duration` tours, puis indisponible `cooldown` tours. VAUTOUR = **{factor 2, duration 2, cooldown 12, paCost 3}** (choix créateur). Fiction : le Vautour gorgé de crédit distressed milke son rendement par à-coups. **Donnée pure** : `Archetype.carrySkill` → ajouter un pouvoir = une ligne. Moteur : `ActorState.{carrySkill, carryBoostUntil, carrySkillReadyAt}` (copiés à l'init, comme `ignoreLockup`) ; `PlannedAction` COMPETENCE (coût prélevé sur le budget PA) ; `executeAction` arme le boost (`carryBoostUntil=t+duration−1`) + pose le cooldown (`readyAt=t+duration+cooldown`) ; `carryBoostMult` multiplie le carry dans `accrueCarryAndCost` (V) **et** `runCreditLifecycle` (coupons). **Inerte si jamais activé** → calibration et autres archétypes intacts. **Contre-poids (règle des pouvoirs)** : 3 PA + cooldown 12 + exposition au défaut crédit en krach → exagéré en magnitude (×2), NEUTRE en espérance. **Validé sur le moteur câblé** (`scripts/vautour-skill.ts`, 400 parties) : top1 vautour **30.8 %→40.5 %**, excédent médian −39 %→+3 %, duel 51 % (§28.8 ✓) — reproduit la mesure-wrapper (×2/cd12 ~40 %). UI : panneau « 🦅 Récolte » (Activer 3 PA · 🔒 prête dans Nt · ● active Nt). **102 tests** (+3 `archetype.test`), svelte-check 0, build OK. **Pipeline archétype prouvé de bout en bout** (idée→mesure→fix baseline→pouvoir→câblage→validation→UI→tests). Reste pour la fiche Vautour §6 : ressource (câbler « Réserve sèche ») + contrainte permanente de cadre |
| 2026-06-14 | **VAUTOUR — 2ᵉ compétence « Couverture » (défensive, armer + auto-tir) → PAIRE off/def co-dosée** : choix créateur d'un duo défensive+offensive (spec §6 : 1-2 mécaniques). **Couverture** : ARMER (2 PA) → anti-défaut des coupons pendant une **fenêtre W tours** → cooldown ; l'**auto-tir** « consomme » la fenêtre quand une crise frappe (pas d'auto-déclenchement sur seuil : on garde la décision joueur = le skill « sentir le danger »). Moteur data-driven : `Archetype.coverSkill {window,cooldown,paCost}` ; `ActorState.{coverArmedUntil,coverReadyAt}` ; `PlannedAction` COMPETENCE skill `cover_arm` ; gate dans `runCreditLifecycle` (armé → `inCreditCrisis=false` pour l'acteur). **Mesure de la fenêtre W** (`scripts/vautour-cover.ts`, armé sur proxy de danger = taux BC élevé) : la PAIRE Récolte(×2)+Couverture **déborde** (Récolte seule déjà 43 %, haut de bande ; W1 44 %, W3 52 %, W6 60 %). **Finding clé : W ne suffit pas — il faut CO-DOSER la paire** (deux pouvoirs partagent le budget de neutralité). **Choix créateur : Option A + W=2** → Récolte dosée vers la RARETÉ (cd 12→**18**, ×2 préservé) + Couverture **W=2** → top1 **~42 %**, duel ~50 % (§28.8 ✓). UI : 2 boutons (Récolte « Activer » / Couverture « Armer ») + état cooldown/actif. Leçon : **garder la magnitude (×2 punch), payer la place de la 2ᵉ compétence par la fréquence**, pas par la fadeur. **104 tests** (+2 cover : protection anti-défaut armée, fenêtre+cooldown), svelte-check 0, build OK. Inerte si non activée → calibration intacte |
| 2026-06-14 | **UI crédit : 2 fixes + logique extraite/testée (option 2)** : au test, deux incohérences sur l'achat de crédit (coupon). (1) **Déplacement** : `openCoupon` ne déplaçait pas le token (ni `playerHex` ni `reveal`) — incohérent avec `open()`. Fix : acheter un coupon sur un émetteur ADJACENT y déplace le joueur (le crédit est traversable) ; émetteur lointain révélé = trade en place (desk à distance), sans téléportation. (2) **CHAIN** : `openCoupon` n'incrémentait pas `opensThisTurn` → après un achat crédit, une position sur un autre actif était traitée comme la 1ʳᵉ ouverture (1 PA). Fix : l'achat de crédit est une ouverture POSITIONNER → il **compte dans le CHAIN** (position suivante sur un autre actif = enchaînement 2 PA) ; coût du crédit lui-même inchangé (1 PA fixe, « appel du desk »). Note : « pas 2 crédits/tour » observé = **émergent** (géographie après déplacement), pas une règle dure ; à formaliser si voulu. **Puis option 2 (testabilité)** : logique d'interaction tricky extraite en **fonctions PURES** `src/lib/interaction.ts` (isInvestable/isCredit/isWalkable, openCost/CHAIN, canOpenAt/canOccupyAt/canMoveToAt/canTradeCouponAt, couponBuyMoves, activateWindow/readyInFromDisplay/activeLeftFromDisplay) — App.svelte **délègue** (source unique : ce qui est testé = ce qui tourne). `src/lib/interaction.test.ts` : **12 tests** hors DOM verrouillant la traversée du crédit, le déplacement à l'achat, le CHAIN, le trade-coupon sans adjacence, le timing des compétences. **116 tests**, svelte-check 0, build OK. Décision : pas d'infra de test de composant pour un proto qui bouge — on extrait/teste la logique pure au cas par cas |
| 2026-06-14 | **VAUTOUR — fiche §6 COMPLÉTÉE (ressource + contrainte)** : ajoute les 2 dernières pièces (§6 : victoire + 1-2 mécaniques + ressource + contrainte). **CONTRAINTE permanente** `noLeverage` (choix créateur) : capital patient → levier toujours rogné à 0 (moteur `executeAction` + UI ; le joueur bypasse le moteur). Friction : ne peut pas amplifier le bull comme le fonds leveragé. **RESSOURCE « Réserve sèche »** (§10, `dryPowder`) : +1 par tour PATIENT (sans ouvrir de position-V, plafond 8), dépensée à l'ouverture en HAUTE FRAGILITÉ (F > 0.55) pour une DÉCOTE d'entrée (≤10 % = 8 × 0.0125) — fidèle au memo « déploiement massif en période de haute fragilité » (achat décoté au creux du krach). Données sur l'archétype, copiées à l'init (comme `ignoreLockup`/skills) ; logique en DOUBLE (moteur pour IA/headless + UI pour le joueur immédiat). **Idée de design : la contrainte PAIE le pouvoir de la ressource** (net neutre). Mesuré (`scripts/vautour-resource.ts`, déployeur de crise) : déployer dans le krach est une ligne FAIBLE (top1 ~16 % à 10 % décote, ~22 % à 20 %) → ressource SANS RISQUE pour la neutralité ; contre-intuitif, `noLeverage` PROTÈGE le déployeur (leviérer un krach = margin call = suicide) — la contrainte mord surtout sur le *bull* (la seule situation où le levier paie). Inerte pour le Vautour-coupons (42 %) et réserve (9 %) → §28.8 intacte. **Effet de bord harnais** : les sondes de PHYSIQUE (`calibration.test.ts`, `calibrate.ts`) doivent passer par l'archétype **NEUTRE** (sans contrainte), sinon un pyromane `steadyLong(4)` voit son levier rogné par le Vautour → plus de crises. UI : jauge « 🦅 Réserve sèche x/8 », levier remplacé par « 🚫 Sans levier ». **120 tests** (+4 : levier rogné, accrual plafonné, décote en krach, pas de décote au calme), svelte-check 0, build OK. Bandes PATH B (8/46/46) + neutralité (duel 55 %) intactes. **Fiche Vautour §6 close** ; reste générique : déploiement des ressources universelles (Réputation), badges (§7), autres archétypes |
| 2026-06-14 | **BC : forward guidance RÉPARÉ (la cible mène le taux) + verdict « lire la BC »** : en explorant le Sismographe, découvert que le forward guidance (présence FED) était MORT — en mode réunions décisives, `bc.target == bc.rate` (plus de lag) → rien à anticiper. Fix : `bcReactionTarget` (pure) recalcule la cible CHAQUE tour (anticipation live de F) ; le taux ne saute à la cible qu'aux réunions → la cible MÈNE le taux ; le rate (tarification coupons) inchangé → calibration intacte. Mesuré (`scripts/sismo-bc-edge.ts`) : pour le TIMING MACRO la BC reste un mauvais signal (aucune forme du taux ne dépasse ~13 % top1 vs 26 % pour F parfait) car la BC COUPE en crise (le taux s'inverse comme jauge de danger) → le Sismographe doit lire F DIRECTEMENT, pas la BC. Détail clé établi : F crée le krach en 3 zones (F<0.40 zone morte = 0 ; 0.40≤F<0.85 roulette = k·(F−0.40)², k 0.7-1.1 ; F≥0.85 plafond = 1) → une roulette, pas un seuil → même voir F donne les ODDS, pas la date (plafond ~26 %) |
| 2026-06-14 | **SISMOGRAPHE — 2ᵉ archétype LIVRÉ (jauge de fragilité + thêta de calme)** : trader macro (réf. Soros §6). **POUVOIR `fragilityGauge`** : voit `F` cachée (jauge sismique innée, EXCLUSIVE — décision design : ne JAMAIS l'offrir à tous, ça tuerait le hidden-F qui est l'âme du jeu §4.4/§28.7 ; structure à 3 étages : signaux bruités universels / nœuds payants / jauge innée premium). Débloque le timing risk-off/risk-on + la frappe all-in au creux. **CONTRAINTE `calmTheta`** (0.5 %/tour) : « fragile au calme » = thêta de couverture (ponction de richesse chaque tour SANS crise active — analogie long-vol/puts qui décaient dans le boom, paient au krach). Clouée sur la crise OBSERVABLE (pas sur F caché) → pas de 2ᵉ mouchard. Données archétype + moteur (`turn.ts` 4quater : `cash -= calmTheta × richesse` hors crise). Mesuré (`scripts/sismo-panel.ts`, `sismo-validate.ts`) : jauge SEULE = outil de SURVIE (top1 26 %, drawdown 14.7→2.8 %) PAS un win-con ; la FRAPPE (déployer 100 % la reprise, débloquée par la jauge) = le moteur (50 % à θ=0) ; le thêta = dial → **θ=0.5 % → top1 38 %, excédent ~0 %, duel 50 % (§28.8 ✓)**. La frappe n'est PAS une mécanique séparée (elle émerge du jeu agressif que la jauge autorise) → archétype = 2 pièces seulement (jauge + thêta). Inerte hors Sismographe → calibration intacte. UI : panneau « 🌋 Jauge sismique » (barre F + zone + état thêta) + ajouté au sélecteur d'archétype. **121 tests** (+1 thêta), svelte-check 0, build OK. **2ᵉ archétype complet** ; le sélecteur UI propose désormais Vautour / Sismographe / Neutre |
| 2026-06-14 | **SISMOGRAPHE rendu ACTIF — « Le Grand Pari » + pas de levier** : retour créateur « il ne serait pas un peu trop passif ? » (juste). Diagnostic : le Vautour a des boutons (Récolte/Couverture), le Sismographe n'avait que de l'info passive (jauge) + un coût subi (thêta) — aucun geste. Mesuré (`scripts/sismo-grandpari.ts`, `sismo-noisy.ts`) : un « Grand Pari » au LEVIER (frappe amplifiée) est **game-breaking** (top1 88-99 %, +15000 % d'excédent) et **ROBUSTE AU BRUIT de la jauge** — cause : leviérer la reprise FIABLE est sans risque quand on time avec la jauge (le levier devient à sens unique). **Le levier est le coupable, pas la jauge.** → Le geste actif doit être DIRECTIONNEL (shorter le krach, à la Soros), pas une amplification : short SANS levier = balançable (θ=0.7 % → top1 38.8 %, excédent +2 %, §28.8 ✓). Build : Sismographe gagne **`noLeverage: true`** (le levier+jauge serait OP, et empièterait sur le créneau du fonds leveragé) ; `calmTheta` 0.005→**0.007** (dose le joueur qui short). Le geste (SHORT le krach) était DÉJÀ dispo dans l'UI — la jauge dit *quand* le tirer → **pas de mécanique séparée** (comme la frappe du Sismo et du Vautour, l'agentivité émerge des primitives + l'info). UI : panneau jauge enrichi (« 🎯 Le Grand Pari : zone rouge → SHORT ou cash · creux → frappe LONG »), levier remplacé par « 🚫 Sans levier ». **121 tests**, svelte-check 0, build OK, calibration intacte. **Leçon transversale : information (quasi-)parfaite + amplification = dégénéré** → l'edge d'un archétype d'info doit rester INFO + DIRECTION, jamais l'amplification |
| 2026-06-14 | **CLÔTURE — VERROU DE PÉRIMÈTRE ajouté (§9bis, choix créateur)** : la fermeture restait gratuite spatialement (on clôturait depuis n'importe où). Correction : **FERMER (1 PA, inchangé) et CLÔTURE PARTIELLE exigent désormais un PÉRIMÈTRE** — on ne pilote la sortie d'une position en `H` que si on est **SUR `H`, ADJACENT, ou dans le MÊME CLUSTER** (adjacence = corrélation, cluster = voisinage corrélé élargi §11) ; au-delà = 🔒 hors de portée. **Asymétrie voulue avec l'OUVERTURE** (qui exige l'adjacence stricte) : on sort d'un peu plus loin qu'on n'entre. **Mis en forme de VERROU + hook de compétence** : nouveau trait données `ignoreClosePerimeter` sur `Archetype` → copié sur `ActorState` à l'init → **fait SAUTER le verrou** (clôture n'importe où, « desk à distance »). Mirroir exact du pattern `ignoreLockup`/`noLeverage`. **Latent** : aucun archétype ne le porte encore → c'est le hook qu'une future compétence activera (le créateur veut « qu'un archétype puisse ENSUITE le faire sauter »). Implémenté en **couche UI** (`interaction.ts` `canActOnPositionAt` pur + testé ; `App.svelte` gate `close`/`partial` + boutons 🔒 + ligne d'explication), cohérent avec l'archi « périmètre/déplacement = UI, moteur position-agnostique » (App.svelte:3). **127 tests** (+6 périmètre), svelte-check 0, build OK. Moteur (`turn.ts`) INCHANGÉ → 60+ tests de physique intacts. À transformer ENSUITE en compétence active (à la Récolte : PA + fenêtre + cooldown) quand on câblera l'archétype porteur |
| 2026-06-14 | **MULTIJOUEUR = BOARD PARTAGÉ (confirmé explicite, §10/§31)** : question créateur « board partagé ou individuel ? » → **un seul plateau commun**. Verrouillé par 3 éléments du design : (1) §10 l.219 — la jauge `F` et le sentiment sont des **états systémiques partagés** que tous *influencent et subissent* (levier/crowding agrégés nourrissent la MÊME fragilité cachée) → des boards séparés videraient le cœur « chaises musicales » ; (2) IA déjà **spatiales/visibles** (footprint sur la carte commune) ; (3) §31.2 *crowding visible* (voir les rivaux converger) n'existe que sur plateau commun. Résolution prévue = **WeGo plan & TICKs** (§31) : choix simultané secret, puis TICKs qui révèlent les **déplacements** (attention) mais cachent les **investissements** (sens/taille) ; prix figés entre TICKs en v1. **Plateau + marché communs, information privée.** Statut : tranché solo-first / multi phase 2 ; §31 = proposition non implémentée. Questions ouvertes (§31.4) : draft de spawn équilibré + conflits simultanés sur un même hexe |
| 2026-06-14 | **PIVOT : esthétique PARQUÉE → focus GAMEPLAY** : après avoir tranché le moteur de rendu (SVG) et brainstormé des pistes de direction artistique (mood terminal/sismographe/clair-obscur, juice CSS, particules de krach), le concepteur **met l'esthétique de côté** (parquée, pas annulée — chantier séparé à reprendre plus tard) pour revenir aux **mécaniques de jeu**. Aucune piste artistique n'est « tranchée » (simples idées en réserve). Prochain pas : nouvelle idée gameplay du concepteur |
| 2026-06-14 | **MOTEUR DE RENDU — TRANCHÉ : SVG (§13, choix créateur)** : on cesse de différer. Le jeu rend ~13-16 hexes *statiques* + jauges + barres + data-viz + texte = profil **tour par tour, UI-lourd**, sans temps réel ni masse d'entités → un moteur « jeu » (Pixi/Phaser) est un **contre-emploi** (boucle de rendu impérative qui casse la réactivité déclarative de Svelte ; texte/data-viz/accessibilité compliqués ; seul gain réel = particules). **Décision : garder SVG** (`App.svelte` déjà ainsi) ; le « juice » via **CSS + transitions/motion Svelte** (remplissage de jauge, pulsation de F, glow d'hexe, fondus). **Escape hatch** : particules de krach un jour ⇒ **overlay PixiJS sur ce seul effet**, jamais un framework qui avale l'UI. **Unity** = build natif futur (portage C# cadré par les 60+ tests). La **direction artistique** (palette/icônes/mood) est orthogonale → chantier séparé. Aucune ligne de code touchée (décision + mémoire) |
