...
describe('combat — attribution du kill (lastHitBy)', () => {
...
describe('combat — Résonance Bastion × Mireille (charge — bonus de déplacement)', () => {
  const mireille = (over: Partial<Unit> & Pick<Unit, 'id' | 'owner' | 'hex'>): Unit =>
    u({ overwatch: { cost: 3 }, range: 4, damage: 2, characterId: 'mireille', ...over });
  const bastion = (over: Partial<Unit> & Pick<Unit, 'id' | 'owner' | 'hex'>): Unit =>
    u({ kind: 'lourde', moveCap: 3, hp: 16, characterId: 'bastion',
        reactions: [{ id: 'ch', on: 'tir_reserve', fromCharacter: 'mireille', scope: { squad: true },
          cooldown: 3, kind: 'charge', amount: 2, duration: 1 }], cooldowns: {}, ...over });

  it('le tir réservé de Mireille charge Bastion (+ CD)', () => {
    const base = makeCombatState(LINE, [
      mireille({ id: 'm', owner: 'alice', hex: 'A', ap: 4 }),
      bastion({ id: 'L', owner: 'alice', hex: 'B', ap: 4 }),
      u({ id: 'x', owner: 'bob', hex: 'E', ap: 4, hp: 20 }),
    ], 'alice');
    const bobTurn = endTurn(reserve(base, 'm'), 4);            // Mireille en guet, bob actif
    const s = resolveOverwatch(moveUnit(bobTurn, 'x', 'C'), 'x'); // E→C → tir → Bastion chargé
    expect(unitById(s, 'L')!.haste).toMatchObject({ owner: 'alice', amount: 2, expiresIn: 1 });
    expect(unitById(s, 'L')!.cooldowns!.ch).toBe(3);
  });

  it('une Lourde chargée dépasse son plafond de déplacement', () => {
    const cap3 = u({ id: 'L', owner: 'alice', hex: 'A', ap: 10, moveCap: 3 });
    expect(moveBudget(cap3)).toBe(3);
    expect(moveBudget({ ...cap3, haste: { owner: 'alice', expiresIn: 1, amount: 2 } })).toBe(5); // 3 + 2
  });

  it('la charge se décompte au tour du possesseur et tombe', () => {
    const s = makeCombatState(LINE, [
      u({ id: 'L', owner: 'alice', hex: 'A', ap: 4, moveCap: 3, haste: { owner: 'alice', expiresIn: 1, amount: 2 } }),
      u({ id: 'b', owner: 'bob', hex: 'E', ap: 4 }),
    ], 'alice');
